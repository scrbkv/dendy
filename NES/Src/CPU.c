...
uint8_t REL() {
	addr_rel = (uint16_t)((int8_t)BUS_cpuRead(pc));
    pc++;
    return 0;
}
...
uint8_t IND() {
    uint16_t ptr_lo = BUS_cpuRead(pc);
    pc++;
    uint16_t ptr_hi = BUS_cpuRead(pc);
    pc++;

    uint16_t ptr = (ptr_hi << 8) | ptr_lo;

    if (ptr_lo == 0x00FF) // Simulate page boundary hardware bug
        addr_abs = BUS_cpuRead(ptr & 0xFF00) << 8 | BUS_cpuRead(ptr + 0);
    else // Behave normally
        addr_abs = BUS_cpuRead(ptr + 1) << 8 | BUS_cpuRead(ptr + 0);
...
uint8_t IZX() {
    ...
    uint16_t lo = BUS_cpuRead((t + x) & 0x00FF);
    uint16_t hi = BUS_cpuRead((t + x + 1) & 0x00FF);
...
uint8_t ASL() {
    ...
    if (lookup[opcode].addrmode == &IMP)
        ...
        BUS_cpuWrite(addr_abs, tmp & 0x00FF);
    return 0;
}
...
uint8_t BRK() {
    ...
    BUS_cpuWrite(0x0100 + s, (pc >> 8) & 0x00FF);
    s--;
    BUS_cpuWrite(0x0100 + s, pc & 0x00FF);
    s--;
    setFlag(B, 1);
    BUS_cpuWrite(0x0100 + s, status);
    s--;
    setFlag(B, 0);

    uint8_t al = BUS_cpuRead(0xFFFE);
    uint8_t ah = BUS_cpuRead(0xFFFF);
...
uint8_t PHA() {
    BUS_cpuWrite(0x0100 + s, a);
    s--;
    return 0;
}

uint8_t PHP() {
    BUS_cpuWrite(0x0100 + s, status | B | U);
...
uint8_t RTI() {
    ...
    s++;
    pc = (uint16_t)(BUS_cpuRead(0x0100 + s));
    s++;
    pc |= BUS_cpuRead(0x0100 + s) << 8;
...
uint8_t RTS() {
    ...
    pc = (uint16_t)(BUS_cpuRead(0x0100 + s));
    s++;
    pc |= (uint16_t)(BUS_cpuRead(0x0100 + s)) << 8;
...
uint8_t STA() {
    BUS_cpuWrite(addr_abs, a);
    return 0;
}

uint8_t STX() {
    BUS_cpuWrite(addr_abs, x);
    return 0;
}

uint8_t STY() {
    BUS_cpuWrite(addr_abs, y);
    return 0;
}
...
void CPU_reset() {
    addr_abs = 0xFFFC;
    uint16_t lo = BUS_cpuRead(addr_abs + 0);
    uint16_t hi = BUS_cpuRead(addr_abs + 1);
...
void CPU_IRQ() {
    if (getFlag(I) == 0) {
        ...
        BUS_cpuWrite(0x0100 + s, status);
        s--;
        uint8_t al = BUS_cpuRead(0xFFFA);
        uint8_t ah = BUS_cpuRead(0xFFFB);
        pc = ((uint16_t)(ah) << 8) | (uint16_t)(al);
        setFlag(I, 1);
        setFlag(B, 0);
        setFlag(U, 1);
...
void CPU_clock() {
    if (CPU_cycle == 0) {
        ...
        opcode = BUS_cpuRead(pc);
        pc++;

        (*lookup[opcode].addrmode)();
        (*lookup[opcode].operate)();
        CPU_cycle = lookup[opcode].CPU_cycles;
    }
    CPU_cycle--;
    global_CPU_cycle++;
}
